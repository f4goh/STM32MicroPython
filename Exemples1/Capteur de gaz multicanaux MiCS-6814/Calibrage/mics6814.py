...
_DEFAULT_I2C_ADDR = const(0x04)

# Version du firmware, valeur à 1126 s'il est à jour en version 2
_ADDR_IS_SET = const(0) 

# Adresse des registres de sortie par défaut (fixés en usine)
_ADDR_FACTORY_ADC_NH3 = const(2)
_ADDR_FACTORY_ADC_CO = const(4)
_ADDR_FACTORY_ADC_NO2 = const(6)

# Adresse des registres de sortie après calibrage utilisateur
_ADDR_USER_ADC_NH3 = const(8)
_ADDR_USER_ADC_CO = const(10)
_ADDR_USER_ADC_NO2 = const(12)

# Adresse I2C du module
_ADDR_I2C_ADDRESS = const(20)

# Commandes du MICS6814
_CH_VALUE_NH3 = const(1)
_CH_VALUE_CO = const(2)
_CH_VALUE_NO2 = const(3)
_CMD_READ_EEPROM = const(6)
_CMD_WRITE_EEPROM = const(7)
_CMD_CONTROL_LED = const(10)
_CMD_CONTROL_PWR = const(11)
...
class MICS6814:
	...
	# Initialisations
	def __init__(self, i2c, addr=_DEFAULT_I2C_ADDR):
		self.i2c = i2c
		self.i2c.scan()
		self.addr = addr
	...
	def get_version(self):
		if self.cmd([_CMD_READ_EEPROM, _ADDR_IS_SET]) == 1126:
	...
	def heater_on(self):
		self.cmd([_CMD_CONTROL_PWR, 1])

	# Arrêt de la chauffe du capteur
	def heater_off(self):
		self.cmd([_CMD_CONTROL_PWR, 0])

	# Allume la LED intégrée du module
	def led_on(self):
		self.cmd([_CMD_CONTROL_LED, 1])

	# Eteint la LED intégrée du mdoule
	def led_off(self):
		self.cmd([_CMD_CONTROL_LED, 0])

	# Affiche les paramètres enregistrés dans l'EEPROM du module
	def display_eeprom(self):
		
		print("_ADDR_I2C_ADDRESS = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_I2C_ADDRESS]))
		print("_ADDR_IS_SET = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_IS_SET]))
		
		print("_ADDR_FACTORY_ADC_NH3 = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_FACTORY_ADC_NH3]))
		print("_ADDR_FACTORY_ADC_CO = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_FACTORY_ADC_CO]))
		print("_ADDR_FACTORY_ADC_NO2 = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_FACTORY_ADC_NO2]))
		
		print("_ADDR_USER_ADC_NH3 = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_NH3]))
		print("_ADDR_USER_ADC_CO = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_CO]))
		print("_ADDR_USER_ADC_NO2 = %d" %self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_NO2]))
		print(" ")
	...
	def get_co(self):
		...
		try:
			A0_1 = float(self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_CO]))
			An_1 = float(self.cmd([_CH_VALUE_CO]))
	...
	def get_no2(self):
		...
		try:
			A0_2 = float(self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_NO2]))
			An_2 = float(self.cmd([_CH_VALUE_NO2]))
	...
	def get_nh3(self):
		...
		try:
			A0_0 = float(self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_NH3]))
			An_0 = float(self.cmd([_CH_VALUE_NH3]))
	...
	def flush_raw(self):
		...
		A0_0 = self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_NH3])
		A0_1 = self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_CO])
		A0_2 = self.cmd([_CMD_READ_EEPROM, _ADDR_USER_ADC_NO2])

		An_0 = self.cmd([_CH_VALUE_NH3])
		An_1 = self.cmd([_CH_VALUE_CO])
		An_2 = self.cmd([_CH_VALUE_NO2])
	...
	def do_calibrate(self):
		...
		while True:
			...
			a0 = self.cmd([_CH_VALUE_NH3])
			a1 = self.cmd([_CH_VALUE_CO])
			a2 = self.cmd([_CH_VALUE_NO2])
			...
			for i in range(20):
				...
				if (a0 - self.cmd([_CH_VALUE_NH3])) > 2 or (self.cmd([_CH_VALUE_NH3]) - a0) > 2:
					cnt += 1
				if (a1 - self.cmd([_CH_VALUE_CO])) > 2 or (self.cmd([_CH_VALUE_CO]) - a1) > 2:
					cnt += 1
				if (a2 - self.cmd([_CH_VALUE_NO2])) > 2 or (self.cmd([_CH_VALUE_NO2]) - a2) > 2:
					cnt += 1
		...
		# Ecriture en EEPROM
		self.cmd([_CMD_WRITE_EEPROM, a0 >> 8, a0 & 0xFF, a1 >> 8, a1 & 0xFF, a2 >> 8, a2 & 0xFF ])