...
# carte NUCLEO-L476RG, en lui ajoutant progressivement des termes (harmoniques).
...
# jusqu'à un oscillogramme très proche d'un signal carré (50 termes max dans notre exemple).
# La broche 'D2' est gérée en interruption, elle enverra les ordre de mesure
# de la à l'analyseur.
...
# La broche 'D2' est en mode numérique. 
# Elle servira à signaler à l'analyseur (ie : la carte NUCLEO-WB55) qu'une nouvelle 
# série de Fourier "tronquée", avec un terme supplémentaire, vient d'être calculée.
TRIGGER_PIN = Pin('D2', Pin.OUT)
TRIGGER_PIN.value(0)
...
def compute_coefs():
	...
	# Précalcul des NB_COEF_MAX premiers coefficients de la série de 
...
# Ils ne seront convertis en entiers codés sur 16 bits (pour programmer le DAC) qu'en
# toute dernière opération, dans la fonctions update_DAC_buffer (ci-après).
...
def compute_square_period():
	...
	for i in range(PERIOD):
		...
		# Série de Fourier avec "approximation sigma" 
...
def update_DAC_buffer():
	...
	# Renormalise la série de Fourier calculée dans sqsig et la l'injecte dans le 
	# tableau d'octets buf. 
...
# Pré-calcule (une seule fois) les NB_COEF_MAX premiers coefficients de la série de 
# Fourier d'un signal carré.
compute_coefs()
		
# Boucle principale, à l'écoute des interruptions du bouton utilisateur via la variable
# globale COMPUTE.
while True:
	...
	if COMPUTE:
		...
		print('Calcul en cours de la nouvelle série, %d coefficients de Fourier' %NB_COEF)
		...
		# Impulsion sur TRIGGER_PIN pour signaler la fin du recalcul
		TRIGGER_PIN.value(1) # Etat 'HAUT' (+3.3V)
		TRIGGER_PIN.value(0) # Etat 'BAS' (0V)
