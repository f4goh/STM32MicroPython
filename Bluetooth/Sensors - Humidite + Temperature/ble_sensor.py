...
_TH_UUID = (bluetooth.UUID('000C0000-0001-11E1-AC36-0002A5D5C51B'), bluetooth.FLAG_NOTIFY) # Humidity + Temperature Char
...
# _FEATURE_MASK :                      100000000011000000000000000000 (en binaire) = 200C0000  (en hexadécimal)

_ST_APP_SERVICE = (_ST_APP_UUID, (_TH_UUID, _SWITCH_UUID))

_PROTOCOL_VERSION   = const(0x01)
_DEVICE_ID          = const(0x80)                           # Generic Nucleo Board
_FEATURE_MASK       = const(0x200C0000)                     # Humidity (2^19), temperature (2^18) and LED (2^29)
...
class BLESensor:
    ...
    # Points d'attention :
    # - Les valeurs doivent être transmises dans l'ordre des ID des caractéristiques (humidité : 19 eme bit, température : 18 eme bit)
    # - Attention à la chaîne de formattage de la fonction Python "pack", égale ici à '<HHh', voir la documentation de "pack"
    def set_data_ht(self, timestamp, humidity, temperature, notify):
        self._ble.gatts_write(self._env_handle, pack('<HHh', timestamp, humidity, temperature))
        if notify:
